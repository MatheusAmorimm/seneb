...
async def get_reports(
    ...
    try:
        entities = await ListReportsUseCase(report_repo, group_repo).execute(
            str(current_user.id), group_id
        )
...
async def get_report_transactions(
    ...
    try:
        entities = await GetReportTransactionsUseCase(
            report_repo, transaction_repo, group_repo
...
async def reopen_report(
    ...
    try:
        await ReopenReportUseCase(report_repo, transaction_repo, group_repo).execute(
...
async def delete_report(
    ...
    try:
        await DeleteReportUseCase(report_repo, transaction_repo, group_repo).execute(
            report_id, str(current_user.id)
        )
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except DomainException as exc:
